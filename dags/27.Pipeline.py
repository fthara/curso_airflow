...
WATERMARK_FILE = EXPORT_DIR / "_watermark_por_estado_json.json"
    ...
    WITH vendas_norm AS (
        SELECT
            ...
            ROUND(
                ...
                ELSE (ve.valor - v.valor_pago) / ve.valor * 100
                END, 2
        ...
        FROM vendas v
        JOIN veiculos ve ON v.id_veiculos = ve.id_veiculos
        JOIN vendedores ven ON v.id_vendedores = ven.id_vendedores
        JOIN concessionarias cons ON v.id_concessionarias = cons.id_concessionarias
        JOIN cidades cid ON cons.id_cidades = cid.id_cidades
        JOIN estados est ON cid.id_estados = est.id_estados
        WHERE est.sigla = %s
...
with DAG(
    ...
    chk_vendas_tem_dados = SQLTableCheckOperator(
        ...
        table='vendas',
        checks={
            "row_count_nonzero": {"check_statement": "COUNT(*)>0"}
        }
    )
    ...
    @task(task_id="export_incremental_por_estado")
    def export_incremental_por_estado() -> None:
        ...
        ufs = [
            ...
            "SELECT DISTINCT sigla FROM estados ORDER BY sigla"
        )]
        ...
        for uf in ufs:
            ...
            df = df.sort_values(
                ["data_venda", "id_venda"]
            ).drop_duplicates(subset=["id_venda"], keep="last")
            ...
            for (ano, mes), part in df.groupby([df["data_venda"].dt.year, df["data_venda"].dt.month]):
                ...
                part.drop(columns=["estado_sigla"], errors="ignore")
    ...
    [chk_vendas_tem_dados, chk_colunas_vendas, chk_integridade_por_join] >> export_incremental_por_estado()
